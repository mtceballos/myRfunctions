    XIFUpixelFracsHMTL <- function(ctrate, deltat, t0, t1, t2, t3, tolsep){
        #
        ...
        # t0, t1, t2, 3, tolsep as in table above
        # fractions (out): list with H(igh)Q, M(edium)Q, T(limiTed)Q, L(ow)Q, I(nvalid)Q, P(i)L(eup) quality
        ...
        if(N.real.photons < 2){  # if less than 2 photons in total
            ...
            for (i in 1:N.real.photons){
                if(i==1){            # first photon case
                    if((points[i+1]-points[i])>t3){  # HQ
                        N.HQ.photons <- N.HQ.photons +1
                    }else if((points[i+1]-points[i])>t2 && (points[i+1]-points[i])<t3){  # MQ
                        N.MQ.photons <- N.MQ.photons +1  
                    }else if((points[i+1]-points[i])>t1 && (points[i+1]-points[i])<t2){  # TQ
                        N.TQ.photons <- N.TQ.photons +1  
            ...
            Frac.TQ <- N.TQ.photons/N.real.photons
        ...
        # Prob of having 0 prev. photons in t0  and     0 next photons in t3
        pH <- dpois(0,ctrate*t0)*dpois(0,ctrate*t3)
        
        # MQ => pM: exp(-ctrate*t0) *  exp(-ctrate*t2)- pH
        # Prob of having 0 prev. photons in t0  and 0 next photons in t2    - pH
        pM <- dpois(0,ctrate*t0)*dpois(0,ctrate*t2)-pH
        
        # TQ => pT: exp(-ctrate*t0) *  exp(-ctrate*t1)- pH
        # Prob of having 0 prev. photons in t0  and     0 next photons in t1    - pH - pM
        pT <- dpois(0,ctrate*t0)*dpois(0,ctrate*t1)-pM - pH
        
        # LQ => pL: exp(-ctrate*t0)*exp(-ctrate*tolsep) -pT
        pL <- dpois(0,ctrate*t0)*dpois(0,ctrate*tolsep)-pM- pH -pT
        ...
        pP <- 1-dpois(0,2*ctrate*tolsep)  
        # also = ppois(0,2*lambda*tolsep, lower.tail=FALSE) -> given 1 photon, which is the prob of finding >0 ph in the double interval?
        pP2 <- 1-dpois(0,ctrate*tolsep)  
        pP1 <- pP - pP2
        # IQ => pI
        pI <- 1-pH-pM-pT-pL-pP
        
        return(list("HQ_ph"=Frac.HQ, "MQ_ph"=Frac.MQ, "TQ_ph"=Frac.TQ,   "LQ_ph"=Frac.LQ, 
                    "IQ_ph"=Frac.IQ, "PL_ph"=Frac.PL, "P1L_ph"=Frac.P1L, "P2L_ph"=Frac.P2L,
                    "HQ_Ps"=pH,      "MQ_Ps"=pM,      "TQ_Ps"=pT,        "LQ_Ps"=pL,      
                    "IQ_Ps"=pI,      "PL_Ps"=pP,      "P1L_Ps"=pP1,      "P2L_Ps"=pP2))
    }
